...
- Functions can be used as user-defined components in JSX within the same scope,
  [as long ase they are capitalized](https://reactjs.org/docs/jsx-in-depth.html#user-defined-components-must-be-capitalized)
  (e.g. `function MyComponent() {...}` becomes `<MyComponent>`). Lowercase tag
  names are passed as string to `React.createElement()`, so it won't work.
- In order to return a list of elements as siblings in the root, use
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=epicreact%40martynus.net).
